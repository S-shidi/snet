VNet 真机验收（当前架构：VPS 协调 + HTTPS + UDP 中继）
========================================================

架构
  - 协调服务器：VPS 66.187.6.46，域名 vnet.uizhi.eu.org:8090（HTTPS，CA 固定）
  - 数据面：WireGuard。优先 NAT 打洞直连，打不通经服务器 UDP 中继（51820..51883）
  - 典型拓扑：Mac（家宽）+ 手机（5G），两侧对称 NAT → 走中继

一、本地回环 e2e（无需外部服务器，验证协调+数据面基线）
  1. 构建：go build -o /tmp/vnet-build/server ./cmd/server
            go build -o /tmp/vnet-build/vnetd ./cmd/client/vnetd
            go build -o /tmp/vnet-build/vnetctl ./cmd/client/vnetctl
  2. 运行（需 root，创建 utun）：
        sudo scripts/e2e.sh
...
二、真机验收（Mac ↔ 手机，经 VPS）
  前置
    - VPS 上 vnet-server 已部署（HTTPS + 中继），见 deploy/README.md
    - 本机 vnetd 已配置：serverAddr=https://vnet.uizhi.eu.org:8090，
      serverCaPath=/usr/local/vnet/certs/server.pem
    - 手机已导入 deploy/phone-android.conf（Endpoint=vnet.uizhi.eu.org:51821，
      PersistentKeepalive=10），且 WireGuard 已被设为不受电池/后台限制

  建网/入网两种方式
    A. 桌面 GUI：创建/加入即连到 VPS（默认 server https://vnet.uizhi.eu.org:8090）。
       注意：GUI「创建网络」会在服务器新建一个网络并把本机切过去——手机须重新
       加入同一网络，否则两端在不同网络互相不可见（2026-08-13 已踩坑）。
    B. 命令行（当前实况，网络 B74P7ZZR / 中继端口 51821）：
        1. 启动/确认本机 daemon：
             sudo launchctl load /Library/LaunchDaemons/com.vnet.daemon.plist
        2. 建网（Mac）：vnetctl --ctl http://127.0.0.1:19432 create \
             --server https://vnet.uizhi.eu.org:8090 --ca-path /usr/local/vnet/certs/server.pem --port 51820
           记录 link（vnet://join?nid=...&code=...）与 Mac 虚拟 IP（10.88.0.1）
        3. 手机（纯 WireGuard 客户端，无 vnet 客户端）用配对码走公开 join 接口
           注册为普通节点（配对码可复用 5 次，无需 admin 令牌）：
             curl -sk -X POST https://vnet.uizhi.eu.org:8090/api/v1/networks/<nid>/join \
        ...
        4. 查该网络中继端口：用本机 daemon token 拉 peers，
           peer.endpoint 即 relayHost:中继端口（B74P7ZZR=51821）：
             curl -sk https://vnet.uizhi.eu.org:8090/api/v1/networks/<nid>/peers \
               -H "Authorization: Bearer <daemon.json 里的 token>"
        5. 把手机配置 Endpoint 端口改成中继端口，qrencode 重出二维码导入

  验收（Mac 侧）
    - vnetctl --ctl http://127.0.0.1:19432 status
  ...
  失败排查
    - status 里 peers 为空：确认 daemon.json serverAddr 域名可达
      （curl -sk https://vnet.uizhi.eu.org:8090/healthz）
