...
SRV=$!
"$BUILD/vnetd" -ctl "127.0.0.1:$CTLA" -config "$TEST/a.json" >"$TEST/a.log" 2>&1 &
DA=$!
"$BUILD/vnetd" -ctl "127.0.0.1:$CTLB" -config "$TEST/b.json" >"$TEST/b.log" 2>&1 &
DB=$!
...
CREATE="$("$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" create --server "http://127.0.0.1:$SRV_PORT" --port 51820 --name "家庭网络")"
echo "$CREATE"
...
CREATE2="$("$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" create --server "http://127.0.0.1:$SRV_PORT" --port 51820 --name "办公网络" --subnet "172.16.77.0/24")"
echo "$CREATE2"
...
COUNT="$( "$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" networks | python3 -c 'import json,sys; print(len(json.load(sys.stdin)))' )"
echo "networks count: $COUNT"

echo "== join net1 (daemon B, second network on same server) =="
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLB" join --server "http://127.0.0.1:$SRV_PORT" --port 51821 --link "vnet://join?nid=$NID1&code=$CODE1"

echo "== netinfo net1 (owner A): expect 2 nodes =="
INFO="$("$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" netinfo --nid "$NID1")"
echo "$INFO"
...
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" leave --nid "$NID2"
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" networks | python3 -c "import json,sys; ns=json.load(sys.stdin); n=[x for x in ns if x['networkId']=='$NID2'][0]; assert not n['active'], 'leave did not stop'"

echo "== rejoin net2 (daemon A) =="
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" rejoin --nid "$NID2"
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" networks | python3 -c "import json,sys; ns=json.load(sys.stdin); n=[x for x in ns if x['networkId']=='$NID2'][0]; assert n['active'], 'rejoin did not activate'"

echo "== reset-code + rename net1 =="
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" reset-code --nid "$NID1"
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" rename --nid "$NID1" --name "家庭网络2"
...
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" kick --nid "$NID1" --node "$B_NODE"

echo "== remove net2 from daemon A =="
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" remove --nid "$NID2"
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" networks | python3 -c "import json,sys; ns=json.load(sys.stdin); assert all(n['networkId']!='$NID2' for n in ns), 'remove failed'"

echo "== remove net1 from daemon B, then owner A deletes it =="
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLB" remove --nid "$NID1"
"$BUILD/vnetctl" --ctl "http://127.0.0.1:$CTLA" delete --nid "$NID1"
