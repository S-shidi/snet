...
cmd/                     Go 入口
  server/                服务端主程序
  client/vnetd/          daemon（macOS launchd / Windows 服务 / Linux 前台）
  client/vnetctl/        命令行管理工具
internal/
...
desktop/                 Tauri 桌面端（Rust + Vite/TypeScript）
  src-tauri/src/vnet/    ctl 封装与各平台服务安装（macos launchd / windows sc.exe）
...
GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -o build/windows-amd64/vnetd.exe ./cmd/client/vnetd
```
