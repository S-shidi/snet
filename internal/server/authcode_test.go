...
func TestAuthCodeLifecycle(t *testing.T) {
	...
	// bind
	if err := s.BindDevice(code1, "dev-aaaa", "pub1"); err != nil {
		t.Fatalf("bind: %v", err)
	}
	...
	if err := s.BindDevice(code1, "dev-bbbb", "pub2"); !errors.Is(err, ErrAuthCodeFull) {
		t.Fatalf("rebind by another device = %v, want ErrAuthCodeFull", err)
	}
	// invalid code
	if err := s.BindDevice("WRONGCODE123", "dev-cccc", "pub3"); !errors.Is(err, ErrAuthCodeInvalid) {
	...
	if err := s.BindDevice(code2, "dev-aaaa", "pub1"); err != nil {
	...
	if err := s.BindDevice(code1, "dev-bbbb", "pub2"); err != nil {
	...
	if err := s.BindDevice(code2, "dev-dddd", "pub4"); err != nil {
	...
	if err := s.BindDevice(code1, "dev-eeee", "pub5"); !errors.Is(err, ErrAuthCodeInvalid) {
...
func TestAuthCodePersistenceAcrossRestart(t *testing.T) {
	...
	if err := s.BindDevice(code, "dev-aaaa", "pub1"); err != nil {
		t.Fatal(err)
	}
	...
	if err := s2.BindDevice(code, "dev-aaaa", "pub1"); err != nil {
...
func TestAuthCodeMultiBind(t *testing.T) {
	...
	if err := s.BindDevice(code, "dev-aaa1", "pub1"); err != nil {
		t.Fatalf("bind 1: %v", err)
	}
	if err := s.BindDevice(code, "dev-bbb2", "pub2"); err != nil {
		t.Fatalf("bind 2: %v", err)
	}
	if err := s.BindDevice(code, "dev-ccc3", "pub3"); err != nil {
		t.Fatalf("bind 3: %v", err)
	}
	// capacity exhausted
	if err := s.BindDevice(code, "dev-ddd4", "pub4"); !errors.Is(err, ErrAuthCodeFull) {
	...
	if err := s.BindDevice(code, "dev-ddd4", "pub4"); err != nil {
...
func TestAuthCodeRotationFreesSlot(t *testing.T) {
	...
	if err := s.BindDevice(codeA, "dev-aaa1", "pub1"); err != nil {
		t.Fatal(err)
	}
	if err := s.BindDevice(codeA, "dev-bbb2", "pub2"); err != nil {
		t.Fatal(err)
	}
	if err := s.BindDevice(codeA, "dev-ccc3", "pub3"); !errors.Is(err, ErrAuthCodeFull) {
		t.Fatalf("A full bind = %v, want ErrAuthCodeFull", err)
	}
	// rotating dev-aaa to codeB releases it from codeA
	if err := s.BindDevice(codeB, "dev-aaa1", "pub1"); err != nil {
		t.Fatal(err)
	}
	if err := s.BindDevice(codeA, "dev-ccc3", "pub3"); err != nil {
		t.Fatalf("A bind after rotation: %v", err)
	}
	// codeB (max 1) is now full
	if err := s.BindDevice(codeB, "dev-ddd4", "pub4"); !errors.Is(err, ErrAuthCodeFull) {
...
func TestAuthCodeMultiBindPersistence(t *testing.T) {
	...
	if err := s.BindDevice(codes[0], "dev-aaa1", "pub1"); err != nil {
		t.Fatal(err)
	}
	if err := s.BindDevice(codes[0], "dev-bbb2", "pub2"); err != nil {
		t.Fatal(err)
	}
	if err := s.Close(); err != nil {
		t.Fatal(err)
	...
	if err := s2.BindDevice(codes[0], "dev-ccc3", "pub3"); !errors.Is(err, ErrAuthCodeFull) {
...
func TestAuthCodeLegacyMigration(t *testing.T) {
	...
	if err := s.BindDevice(codes[0], "dev-old1234", "pubO"); err != nil {
		t.Fatal(err)
	}
	if err := s.Close(); err != nil {
		t.Fatal(err)
...
func TestAdminAuthCodeEndpoints(t *testing.T) {
	...
	if err := s.BindDevice(gen2.Codes[0], "dev-m1abc", "pubm1"); err != nil {
		t.Fatal(err)
	}
	if err := s.BindDevice(gen2.Codes[0], "dev-m2abc", "pubm2"); err != nil {
		t.Fatal(err)
	}
	resp = doJSON(t, http.MethodPost, ts.URL+"/api/v1/devices/bind", "",
	...
	if err := s.BindDevice(gen.Codes[0], "dev-aaaa", "pub1"); err != nil {
		t.Fatal(err)
	}
...
func TestBindConcurrent(t *testing.T) {
	...
	for i, dev := range []string{"dev-aaa1", "dev-aaa2"} {
		...
		go func(i int, dev string) {
			defer wg.Done()
			<-start
			errs[i] = s.BindDevice(code, dev, "pub")
		}(i, dev)
	}
	close(start)
	wg.Wait()
